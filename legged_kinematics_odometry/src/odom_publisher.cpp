...
public:
        ...
        : Node("odom_publisher_node")
    {
        ...
        // ================================================
        // Get odom from VO and integrate with leg odom
        // ================================================
        odom_kinematics_publisher_ = this->create_publisher<nav_msgs::msg::Odometry>("/odom_kinematics", 10);
        ...
        kinematics_integrator_timer_ = this->create_wall_timer(
            20ms, std::bind(&OdomPublisherNode::publishKinematicsOdom, this));
    
        // Set the initial time stamp for integration
        kinematics_odom_last_update_time_ = this->now();
    
        // Initialize the integrated transform to identity.
        current_odom_kinematics_.header.frame_id = "odom_kinematics";
        current_odom_kinematics_.child_frame_id = base_frame_name_;
        current_odom_kinematics_.transform.translation.x = 0.0;
        current_odom_kinematics_.transform.translation.y = 0.0;
        current_odom_kinematics_.transform.translation.z = 0.0;
        tf2::Quaternion q;
        q.setRPY(0, 0, 0);
        current_odom_kinematics_.transform.rotation = tf2::toMsg(q);
    }
...
private:
    ...
    void publishKinematicsOdom()
    {
        // Get time difference for integration
        auto current_time = this->now();
        double dt = (current_time - kinematics_odom_last_update_time_).seconds();
        kinematics_odom_last_update_time_ = current_time;

        // Try to look up the transform from "odom" to base_frame_name_
        try {
            // Check if there is a new transform available.
            // and transform it to the odom_kinematics_frame
            geometry_msgs::msg::TransformStamped odom_to_base =
                tf_buffer_->lookupTransform("odom", base_frame_name_, tf2::TimePointZero);

            if (odom_to_base.header.stamp != last_received_odom_base_transform_stamp_)
            {
                // RCLCPP_INFO(this->get_logger(), "New transform available, resetting integration.");
                // New transform available, reset the integration.
                geometry_msgs::msg::TransformStamped odom_kinematics_to_odom =
                    tf_buffer_->lookupTransform("odom_kinematics", "odom", tf2::TimePointZero);
    
                geometry_msgs::msg::TransformStamped odom_kinematics_to_base =
                    multiplyTransforms(odom_kinematics_to_odom, odom_to_base);
    
                // Reset our integrated transform with the new value.
                current_odom_kinematics_ = odom_kinematics_to_base;
                current_odom_kinematics_.header.frame_id = "odom_kinematics";
                current_odom_kinematics_.child_frame_id = base_frame_name_;

                // Update the last received transform stamp
                last_received_odom_base_transform_stamp_ = odom_to_base.header.stamp;
            }
            else
            {
                // RCLCPP_INFO(this->get_logger(), "No new transform available, performing integration.");
                // No new transform, perform integration
                // Convert current_odom_kinematics_ into a Pinocchio SE3 object.
                Eigen::Vector3d t(current_odom_kinematics_.transform.translation.x,
                    current_odom_kinematics_.transform.translation.y,
                    current_odom_kinematics_.transform.translation.z);
                tf2::Quaternion tf_q;
                tf2::fromMsg(current_odom_kinematics_.transform.rotation, tf_q);
                ...
                current_odom_kinematics_.header.stamp = latest_twist_.header.stamp;
                current_odom_kinematics_.transform.translation.x = new_translation.x();
                current_odom_kinematics_.transform.translation.y = new_translation.y();
                current_odom_kinematics_.transform.translation.z = new_translation.z();
                current_odom_kinematics_.transform.rotation = tf2::toMsg(new_tf_q);
            }
            ...
            current_odom_kinematics_.header.stamp = latest_twist_.header.stamp;
            tf_broadcaster_->sendTransform(current_odom_kinematics_);
        }
        ...
        try
        {
            ...
            // Publish the odometry message (not necessary for the system but can be visualized)
            nav_msgs::msg::Odometry kinematics_odom_msg;
            kinematics_odom_msg.header = current_odom_kinematics_.header;
            kinematics_odom_msg.header.frame_id = "odom";
            kinematics_odom_msg.child_frame_id = base_frame_name_;
            
            // We have to transform it to the odom frame to be visualized
            geometry_msgs::msg::TransformStamped odom_to_odom_kinematics =
                        tf_buffer_->lookupTransform("odom", "odom_kinematics", tf2::TimePointZero);

            geometry_msgs::msg::TransformStamped odom_kinematics_to_base_odom_frame =
                        multiplyTransforms(odom_to_odom_kinematics, current_odom_kinematics_);

            // Fill in the pose from your integrated transform
            kinematics_odom_msg.pose.pose.position.x = odom_kinematics_to_base_odom_frame.transform.translation.x;
            kinematics_odom_msg.pose.pose.position.y = odom_kinematics_to_base_odom_frame.transform.translation.y;
            kinematics_odom_msg.pose.pose.position.z = odom_kinematics_to_base_odom_frame.transform.translation.z;
            kinematics_odom_msg.pose.pose.orientation = odom_kinematics_to_base_odom_frame.transform.rotation;
            odom_kinematics_publisher_->publish(kinematics_odom_msg);
        }
        catch(tf2::TransformException &ex)
        {
            ...
            nav_msgs::msg::Odometry kinematics_odom_msg;
            kinematics_odom_msg.header = current_odom_kinematics_.header;
            kinematics_odom_msg.header.frame_id = "odom_kinematics";
            kinematics_odom_msg.child_frame_id = base_frame_name_;
            kinematics_odom_msg.pose.pose.position.x = current_odom_kinematics_.transform.translation.x;
            kinematics_odom_msg.pose.pose.position.y = current_odom_kinematics_.transform.translation.y;
            kinematics_odom_msg.pose.pose.position.z = current_odom_kinematics_.transform.translation.z;
            kinematics_odom_msg.pose.pose.orientation = current_odom_kinematics_.transform.rotation;
            odom_kinematics_publisher_->publish(kinematics_odom_msg);
    ...
    // Get odom from VO and integrate with leg odom
    // ================================================
    rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr odom_kinematics_publisher_;
    rclcpp::TimerBase::SharedPtr kinematics_integrator_timer_;
    
    std::shared_ptr<tf2_ros::Buffer> tf_buffer_;
    std::shared_ptr<tf2_ros::TransformListener> tf_listener_;

    // The latest transform stamp to check if a new transform has been broadcast
    rclcpp::Time last_received_odom_base_transform_stamp_;
    
    // Latest twist (velocity) message
    geometry_msgs::msg::TwistWithCovarianceStamped latest_twist_;
    // For integration time delta
    rclcpp::Time kinematics_odom_last_update_time_;
    // The integrated transform from "odom_kinematics" to "base_link"   
    geometry_msgs::msg::TransformStamped current_odom_kinematics_;
};
