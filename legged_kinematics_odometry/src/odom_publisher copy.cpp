...
public:
        ...
        : Node("odom_publisher_node")
    {
        ...
        current_odom_kinematics_map_.child_frame_id = "base_link";
...
private:
    ...
    // update the integrated transform from odom->base_link and publish it
    void publishMapKinematics()
    {
        ...
        // and use it (together with odom->map) to reset the integration.
        try {
        // Get the latest transform from map -> base_link.
        geometry_msgs::msg::TransformStamped map_to_base =
            tf_buffer_.lookupTransform("map", base_frame_name_, tf2::TimePointZero);

        // Look up the transform from odom -> map.
        geometry_msgs::msg::TransformStamped odom_to_map =
            tf_buffer_.lookupTransform("odom", "map", tf2::TimePointZero);

        // Compute the transform from odom -> base_link:
        //   T_odom_base = T_odom_map * T_map_base
        geometry_msgs::msg::TransformStamped odom_to_base =
            multiplyTransforms(odom_to_map, map_to_base);

        // Reset our integrated transform with the new value.
        // (This means we integrate on top of the latest available measurement.)
        current_odom_kinematics_map_ = odom_to_base;
        current_odom_kinematics_map_.header.frame_id = "odom_kinematics_map";
        current_odom_kinematics_map_.child_frame_id = "base_link";
        } catch (tf2::TransformException &ex) {
        RCLCPP_WARN(this->get_logger(), "Transform lookup failed: %s", ex.what());
        // If the transform isn’t available, we continue integrating from the previous state.
        }

        // --- Integration step ---
        // Assume the twist is expressed in the base_link frame.
        // Compute the incremental translation and rotation over the time interval.
        double dx = latest_twist_.twist.twist.linear.x * dt;
        double dy = latest_twist_.twist.twist.linear.y * dt;
        double dtheta = latest_twist_.twist.twist.angular.z * dt;

        // Extract the current pose (x, y, yaw) from the integrated transform.
        double x = current_odom_kinematics_map_.transform.translation.x;
        double y = current_odom_kinematics_map_.transform.translation.y;

        tf2::Quaternion q;
        tf2::fromMsg(current_odom_kinematics_map_.transform.rotation, q);
        double roll, pitch, yaw;
        tf2::Matrix3x3(q).getRPY(roll, pitch, yaw);

        // Since the twist is in the base_link frame, rotate the incremental
        // translation into the global frame (using the current yaw)
        double new_x = x + (dx * std::cos(yaw) - dy * std::sin(yaw));
        double new_y = y + (dx * std::sin(yaw) + dy * std::cos(yaw));
        double new_yaw = yaw + dtheta;

        // Update the stored transform with the integrated pose.
        current_odom_kinematics_map_.transform.translation.x = new_x;
        current_odom_kinematics_map_.transform.translation.y = new_y;
        current_odom_kinematics_map_.transform.translation.z = 0.0;  // Assuming a planar robot

        tf2::Quaternion new_q;
        new_q.setRPY(0, 0, new_yaw);
        current_odom_kinematics_map_.transform.rotation = tf2::toMsg(new_q);
        current_odom_kinematics_map_.header.stamp = current_time;

        // Publish the integrated transform between "odom_kinematics_map" and "base_link"
        transform_pub_->publish(current_odom_kinematics_map_);
    }

    // Helper function to multiply two transforms (i.e. perform a transformation composition)
        ...
        const geometry_msgs::msg::TransformStamped & B)
    {
        ...
        result.header.stamp = A.header.stamp;  // Use A’s timestamp (or update as needed)
